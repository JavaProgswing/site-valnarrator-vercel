...
def failure_template(
    title: str, message: str, button_text: str = "Return Home", button_link: str = "/"
) -> str:
    return get_base_template(
        title, message, "error", "fa-solid fa-circle-xmark", button_text, button_link
    )
...
async def handle_referral_apply(
    ...
    if not user_id or not referral_code:
        return HTMLResponse(
            content=failure_template(
                "Invalid Request",
                "Missing referral code or user ID!",
                "Try Again",
                retry_link,
            ),
    ...
    if not referral_record:
        return HTMLResponse(
            content=failure_template(
                "Invalid Referral", "Referral code not found!", "Try Again", retry_link
            ),
    ...
    if not user_exists:
        return HTMLResponse(
            content=failure_template(
                "Invalid User",
                "User ID does not exist in our records.",
                "Try Again",
                retry_link,
            ),
